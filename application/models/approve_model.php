...
class Approve_model extends MY_Model{
	...
	public function list_page($budget_main_ID, $where) {
		...
		if(!empty($where))
		    $this->db->like('trn_approve.doc_number', $where);
    ...
    public function num_page($budget_main_ID, $where) {
		...
		if(!empty($where)){
		   $result = $this->db->from('trn_approve')->where('trn_approve.budget_main_id', $budget_main_ID)
		   	         ->like('trn_approve.doc_number', $where)
		             ->count_all_results();
		}else
	...
	function search_doc_number($budget_main_ID, $q)
	{
		...
		return $this->db->query("select trn_approve.id, trn_approve.doc_number from $table_db where trn_approve.budget_main_id = '$budget_main_ID' AND locate('$q', $find_field) > 0 group by trn_approve.doc_number order by locate('$q', $find_field), $find_field limit $pagesize");
	}
    
    // search.php
    function search_docNumber_paymented($budget_main_ID, $q)
	{
	    $q = urldecode($q);
		$pagesize = 50; 
		$table_db = "trn_approve"; 
		...
		return $this->db->query("select trn_approve.id, trn_approve.doc_number from $table_db where trn_approve.budget_main_id = '$budget_main_ID' AND trn_approve.status+0 != 1 AND locate('$q', $find_field) > 0 group by trn_approve.doc_number order by locate('$q', $find_field), $find_field limit $pagesize");
	}
	...
	function search_file_number($budget_main_ID, $q)
	{
		...
		return $this->db->query("select trn_approve.id, trn_approve.file_number from $table_db where trn_approve.budget_main_id = '$budget_main_ID' AND locate('$q', $find_field) > 0 group by trn_approve.file_number order by locate('$q', $find_field), $find_field limit $pagesize");
	}
    ...
    public function approve_only_by_budget($budget_main_ID) {
       // $this->db->select('trn_approve.mgt_costs_id'); 
		...
		$this->db->where('trn_approve.status =', 1);
        $this->db->where('trn_approve.budget_main_id =', $budget_main_ID);
    ...
    function approve_paymented($budget_main_ID, $where)
	{
     ...
     public function num_paymented_report($budget_main_ID, $where) {
        $this->db->select('trn_approve.id'); 
        $this->db->select('IFNULL(SUM(trn_payment.amount),0)  AS payment',FALSE);
        $this->db->join('trn_disbursement','trn_disbursement.approve_id = trn_approve.id','LEFT OUTER');
        $this->db->join('trn_payment', 'trn_payment.disbursement_id = trn_disbursement.id','LEFT OUTER');
